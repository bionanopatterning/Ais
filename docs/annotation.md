...
Segmenting tomographic volumes comprises a number of processing steps: i) manual annotation, ii) training neural networks, iii) applying the neural networks to the volumes. For each of these steps, a slightly different interface is available. After starting the software and importing a dataset (either by dragging a file and dropping it onto the Ais window, or via the top menu bar File → Import dataset), the interface looks as follows:

<figure markdown="span">
  ![The user interface upon starting the software and importing a single tomogram](res/annotation_1.png){ .with-border }
  <figcaption>Figure 1 – the user interface upon starting the software and importing a single tomogram.</figcaption>
</figure>
...
Use the 'Add feature' button to set up three features: membrane, carbon, and antibody platform, as below.

<figure markdown="span">
  ![The user interface in the Segmentation stage, with three features set up and annotated](res/annotation_2.png){ .with-border }
  <figcaption>Figure 2 – the user interface in the 'Segmentation' stage, with three features set up and annotated.</figcaption>
</figure>
...
| **Set active feature** | click it in the Features panel, or use the W/S keys |
...
<figure markdown="span">
  ![Placing positive and negative boxes for the antibody platforms segmentation](res/annotation_3.png){ .with-border }
  <figcaption>Figure 3 – placing positive and negative boxes for the antibody platforms segmentation.</figcaption>
</figure>

In order for a neural network to be able to accurately distinguish between the intended feature of interest and other, possibly similarly looking, features, it is important to select positive boxes as well as negative boxes. In the example above, we've placed 6 positive boxes as well as a number of negative boxes: boxes that contain no annotation for the antibody platforms and that do not contain antibody platforms either.
...
<figure markdown="span">
  ![The annotations and boxes used to generate the models discussed in main text figure 2](res/annotation_4.png){ .with-border }
  <figcaption>Figure 4 – the annotations and boxes used to generate the models discussed in main text figure 2.</figcaption>
</figure>

## Manual particle picking and segmentation

Ais can also be used for manual particle picking. By hovering over the '>' icon in the bottom right of every feature panel, a menu opens in which annotated slices are listed, and from where some files can be exported: i) tab-separated coordinates `.txt` files, ii) single annotations can be saved as a 2D `.mrc` file, and iii) a full annotated volume can be exported as a 3D `.mrc` file.
